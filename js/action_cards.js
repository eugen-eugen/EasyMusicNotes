...
function Random_buttons(params) {
    ...
    for (let notenNummer = 0; notenNummer < Noten.length /*5*/; notenNummer++){ //Die Schleife läuft über alle Noten
        ...
        bildDatei= Img.src; // Die Bilddatei wird genommen
        notenName=bildDatei.substr (113,3).replace (".", ""); // Aus der Bilddatei wird der Notenname rausgeschnitten
...
function notePressed(p) {
    ...
    src= markedImg.src;
    markedNoteName=src.substr (113,3); 
...
if (markedNoteName.replace (".", "")==pressedNoteName){// Es wird überprüft, ob die richtige Note gedrückt wurde.
